...
\newcommand{\tanilson}[1]{\textcolor{auburn}{\sf{#1}}}% es para agregar comentarios en marron
...
\tanilson{In next section we explore the Chordal $B_1$-EPG graphs through of a subset of forbidden graphs and we will proof that this class is in the strict intersection of VPT and EPT graphs.}
...
\end{document}
por si acaso, aqui dejo la versión anterior
...
we have that $I_{1,4}$ must be contained in $P_2$, which makes impossible the existences of a path  $P_7$ intersecting 4 and 2, but not 1. 
 \hfill $\square$\end{pf} }
***********************************************************
...
The graph $F_7$ on Figure~\ref{fig:16proibidos}(g) is not   $B_1$-EPG.
\end{lema}
\begin{pf} Suppose in order to derive a contradiction that there is a  $B_1$-EPG representation  of $F_7$. For $i\in \{4,5,6\}$, let $I_{1,i}$ be the subpath of $P_1$
defined to be $P_1\cap P_i$ (we  consider paths as edge sets). Clearly, the three
subpaths are pairwise edge-disjoint. Thus, w.l.o.g., by symmetry,\la{aqui no hay simetría}  we can assume that
$I_{1,5}$ is between  $I_{1,4}$ and $I_{1,6}$. We claim that the clique $\{1,2,3,5\}$ is an edge-clique, see Lemma~\ref{lem:3cliquesNotClaw} and Lemma~\ref{coro:3Cliques1EdgeClique}.
We can assume, w.l.o.g., that $\{1,2,3,4\}$ is an edge-clique, which implies that $P_2$ and $P_3$ contain at least an edge of $I_{1,4}$. Since $P_6$
contains an edge of $I_{1,6}$ and cannot intersect $P_5$, by Fact \ref{f:between}, $P_2$ and $P_3$  contain an edge of $I_{1,5}$, which implies that $\{1,2,3.5\}$ is an edge-clique, as we wanted to proved.\\
Now, since  $\{1,2,3,5\}$ is an edge-clique and $P_2$ and $P_3$ have that to intersect $P_4$ on one side and $P_6$ on the other side of $I_{1,5}$,
we have that $I_{1,5}$ must be contained in $P_2$ and $P_3$, which makes impossible the existences of a path  $P_8$ intersecting $P_5$ and $P_2$, but not $P_1$ and $P_3$. 
\hfill $\square$\end{pf} }
...
\begin{pf}
By Lemma~\ref{lem:3cliquesNotClaw} the maximal cliques $K_1, K_2$ and $K_3$ can not be represented simultaneously as claw-clique and by Lemma~\ref{coro:3Cliques1EdgeClique} there is one edge-clique $K_i$ whose two paths representing satellites of the independent set $I$ are in different sides of  $K_i$. When $K_2$ is represented by claw-clique, the path $P_2$, corresponding to vertex 2, belongs to claw-clique, so it contains two edges of this claw. As $P_1$ and $P_3$, corresponding respectively to the vertices 1 and 3,  do not intersect $P_2$ then only one side of this clique remains to $K_1$ and $K_3$. Thus $K_1$ and $K_3$ are on the same side of $K_2$ and the lemma holds.
\la{la definición de un lado y el otro es para edges cliques y no para claw cliques...}
\tanilson{reescrevi esse trecho, observe se esta melhor...}
...
%Let $C_{B'}$ be the subgraph obtained by $C_{B}-\{n\}$ and $R'$ its $B_1$-EPG representation, where the satellites $1$ and $3$ are in disjoint groups relative to edge-clique $K_2$. 
 ...
 where paths $P_1$ and $P_3$ are in distinct sides of $K_2$. \hfill $\square$
...
$I_2=\{1,2,3\}$. Such that each vertex of $K$ is adjacent to each vertex of $I_1$, and each  vertex $i \in I_2$ is adjacent to $w_i$ and to a proper subset $S_i$ of $K$. Then, the graph $G$ is not a $B_1$-EPG graph.
\end{lema}

\begin{pf}
Since the induced subgraph $G[K\cup I_1]$ is isomorph to the graph of Lemma~\ref{lem:3cliquesNotClaw}, then at least one clique $K_i = \{K \cup \{w_i\}\}$, where $w_i \in I_1$ is represented by edge-clique. By Lemma~\ref{coro:3Cliques1EdgeClique}, this edge-clique is between two paths representing satellites $w_i$. Whatever be the $K_i$ chosen, in the induced subgraph $G[K\cup I_1 \cup \{i\}]$, with $i\in I_2$, it will be in the same restriction conditions as described in Lemma~\ref{lem:obstrucaoCentro} and there is no a $B_1$-EPG representation for this construction. Therefore, $G$ is not a  $B_1$-EPG graph.
\hfill $\square$\end{pf}
...
\end{lema}
\begin{pf}
The graph $F_6$ (see Figure~\ref{fig:16proibidos}(f)) has as induced subgraph a graph that can be obtained by construction of the Lemma~\ref{lem:obstrucaoGeneralizada}. Thus, $F_6$ also is not a $B_1$-EPG graph.
\hfill $\square$\end{pf}} 
...
\end{lema}
\begin{pf}
The graph $F_7$ (see Figure~\ref{fig:16proibidos}(g)) has as induced subgraph a graph that can be obtained by construction of the Lemma~\ref{lem:obstrucaoGeneralizada}. Thus, $F_7$ also is not a $B_1$-EPG graph.
\hfill $\square$\end{pf} }
...
Let $G$ be the graph with the same set of vertices of $F_{8}$ (see Figure~\ref{fig:16proibidos}(h)) and the same set of edges except by $e_1=(4,6)$ and   $e_2=(8,6)$. Then, the graph $G$  is not a $B_1$-EPG graph. 
\end{lema}

\begin{pf}
We know that each clique in a single bend EPG representation is represented by edge-clique or claw-clique. In the graph $G$ %reemplazo F_10(8)' por R para facilitar la notación
...
\end{coro}
\begin{pf}
The proof is immediate by Lemma~\ref{lem:F10-8opcIsNotB1EPG}.  \hfill $\square$
\end{pf}
