...
\la{It's known that not every $B_1$-EPG graph admits a Helly representation. We are interested in determining the subgraphs that make
...
\la{Notice that if three vertices induce a claw-clique, then exactly two of them turn at the center of the corresponding  claw of the grid, and the third one contains the
base of the claw. 
...
Let $G$ be a $B_1$-EPG graph. If $G$ is $C_4$-free, then there exists a $B_1$-EPG representation of $G$ such that every \la{maximal} claw-clique $K$ is represented on a claw of the grid whose base is covered only by vertices of $K$.
\end{lema}
...
In this section, we delimit some  subclasses of $B_1$-EPG graphs that admit a Helly representation. It is known that $B_1$-EPG and Helly $B_1$-EPG 
...
those problems,  we describe a few structures that provide a $B_1$-EPG graph does not admit a Helly $B_1$-EPG representation, that is it is not a  Helly $B_1$-EPG graph. In addition,
...
\la{A $B_1$-EPG intersection model and the graph depicted in Figure~\ref{fig:clawGrid} have edges that may or may not occur (dashed edges are optional)}. This figure is useful for the proof of Theorem~\ref{lem:chordalDiamondFree}.
...
\la{A graph $G$ is said to be \textit{Bipartite} if and only if there exists a partition $V(G)=A\cup B$ and $A\cap B=\emptyset$. Hence, all edges share a vertex from both set $A$ and $B$, and there are no edges formed between two vertices in the set $A$, and there are not edges formed between the two vertices in $B$.}
